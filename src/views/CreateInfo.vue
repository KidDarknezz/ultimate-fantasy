...
export default {
    ...
    methods: {
        importData: function() {
            ...
            for (let j in jsonImportedData) {
                for (let i in this.$store.state.allPlayers) {
                    if (
                        this.$store.state.allPlayers[i].playerId ==
                        jsonImportedData[j].playerId
                    ) {
                        this.patchScore.score =
                            this.$store.state.allPlayers[i].score +
                            jsonImportedData[j].score
                    }
                }
        ...
        patchNewScore: function(playerId) {
            this.$http
                .patch(
                    `https://ultimate-fantasy-fe04f.firebaseio.com/players/${playerId}.json`,
                    this.patchScore
                )
                .then(response => {
